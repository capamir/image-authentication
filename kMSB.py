...
def extract_msb(image_path, k):
    ...
    for y in range(height):
        for x in range(width):
            ...
            pixel_value = new_img.getpixel((x, y))
    ...
    folder_path = "kmsb-imsge"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
        
    # Save the new image in the specified folder
    new_img_path = os.path.join(folder_path, f"new_image_with_msb_{k}.jpg")
