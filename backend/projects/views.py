...
def uploadImage(request):
    try:
        ...
        # Normalize the reconstructed image to ensure the data is constant
        ...
        # Encode bytes within dictionaries and serialize dictionaries to JSON strings and encode to base64
        ...
        # saving image hash in block chain
...
class ImageAddressViewSet(viewsets.ViewSet):
    def create(self, request):
        ...
        # import orginal image's data from block chain
        block_index = index
        block = Block.objects.get(index=block_index)
        orginal_hash = eval(block.column_hash)

        # extracting channels and decoding base64
        ...
        if(differences != []):
            ...
            # reconstructing channel Y of the restored image
            restored_y_channel = reconstruct_y_channel_image(updated_columns_dict, block_size, image.shape)
          

            # Reconstructing the Cb and Cr channels of the orginal iamge
            ...
            # Merging the restored Y, Cb, and Cr channels to form the final image
            restored_image_ycrcb = cv2.merge([restored_y_channel, restored_cb_channel, restored_cr_channel])
            ...
            'compressed_image': highlighted_image_base64,
             "message":" The image succesfully restored", 
                                                              }, status=status.HTTP_200_OK)
        else:
